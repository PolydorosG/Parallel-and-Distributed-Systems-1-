...
/*Given the .mtx file this function extracts (i,j) of non zeroes*/
...
int main(int argc, char *argv[]){
    ...
    /*2 outer for loops are used for benchmarking change if running one test*/
    for(int max_threads = 2; max_threads < 17 ; max_threads++){
        sprintf(workers, "%d", max_threads);
        __cilkrts_end_cilk();
